...
d3.json("wis.json", function(error, graph) {
  ...
  var label = container.append("g")
    ...
    .data(graph.nodes)
    .enter().append("label")
    // create rect to hold textRemove
  label.append("text")
...
/*
  ...
  d3.selectAll('label rect')
    ...
    .attr('y', function (d) {
      return d.labelBBox.y;
    })

    simulation.alphaTarget(0).restart();*/

  // THIS WORKS, SORT OF - WILL LOG BBOX DIMENSIONS
  ...
  nodeRect = d3.selectAll("label rect")
    .attr("x", bbox.x)
    .attr("y", bbox.y)
  ...
  /*var bbox = d3.selectAll('text').each(function(d, i) {
     d.bbox = this.getBoundingClientRect()
     console.log('I exist!')
     console.log(d.bbox);
  });
