...
d3.json("wis.json", function(error, graph) {
  ...
  var nodeLabels = container.append('g').attr('class', 'labels')
    .selectAll('text')
    .data(graph.nodes)
    .enter()
    .append('text')
    .text(function(d) { return d.name })
    .style('fill', '#555') // make more prominent on mouseover 
    .style('pointer-events', 'none'); // prevent mouseover/drag capture
