...
d3.json("wis.json", function(error, graph) {
  ...
  // Create variable to store all bbox data
  var bbox = d3.selectAll('.label text').call(getBBox)

  // Get bbox. Logging for debugging purposes - delete later
  ...
  // Insert rect to hold text with dimensions of Bbox

  var textBackground = label.insert('div', 'label text')
    .attr({
        'x': d => d.bbox.x,
        'y': d => d.bbox.y,
